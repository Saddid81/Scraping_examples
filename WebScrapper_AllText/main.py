...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            db = dbConn['crawlerDB1'] # connecting to the database called crawlerDB
            ...
            if reviews.count() > 0: # if there is a collection with searched keyword and it has records in it
                ...
                allcommentsLink = "https://www.flipkart.com" + prod_html.findAll('div', {'class':"_3UAT2v _16PBlm"})[0].previous_element.attrs['href']
                allcommentsRes = requests.get(allcommentsLink)
                ...
                for i in range(1, pag_max_num+1):
                    link= baseLink + str(i)
                    linkRes = requests.get(link)
                    link_html = bs(linkRes.text, "html.parser")

                    allcomments_boxes = link_html.find_all('div', {'class':"_1AtVbE col-12-12"})

                    #  iterating over the comment section to get the details of customer and their comments
                    for commentbox in allcomments_boxes[4:14]:
